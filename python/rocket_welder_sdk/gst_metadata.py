...
class GstCaps:
    ...
    channels: int
    bytes_per_pixel: int
    framerate_num: Optional[int] = None
    framerate_den: Optional[int] = None
    interlace_mode: Optional[str] = None
    colorimetry: Optional[str] = None
    caps_string: Optional[str] = None
    ...
    def framerate(self) -> Optional[float]:
        ...
        if self.framerate_num is not None and self.framerate_den is not None and self.framerate_den > 0:
    ...
    @staticmethod
    def _map_gstreamer_format_to_numpy(format: str) -> Tuple[np.dtype, int, int]:
        """
    ...
    def create_array(self, data: bytes | memoryview | np.ndarray) -> np.ndarray:
        """
    ...
    def create_array_from_pointer(self, ptr: int, copy: bool = False) -> np.ndarray:
        """
        ...
        arr = np.frombuffer(
            np.core.multiarray.int_asbuffer(ptr, total_elements * self.depth_type.itemsize),
            dtype=self.depth_type
        )
